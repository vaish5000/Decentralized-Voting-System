    ...
    App.contracts.Election.deployed().then(function (instance) {
      ...
      for (var i = 1; i <= candidatesCount; i++) {
        electionInstance.candidates(i).then(function (candidate) {
          ...
          /*if(voteCount.tonumber()>max.tonumber()){
            max.tonumber()=voteCount.tonumber();
            maxindex.tonumber()=i.tonumber();
          }*/
        });
      }
      /*electionInstance.candidates(1).then(function (candidate) {
        ...
        var voteCount = candidate[3];
        var party = candidate[2];
        candidatesResults.append("Results:");
        var candidateTemplate =
          `<tr><th>${id}</th><td>${name}</td><td>${party}</td><td>has higest votes</td></tr>`;
        candidatesResults.append(candidateTemplate);
        var candidateOption = `<option value="${id}"> ${name} (${party}) </option>`
        candidatesSelect.append(candidateOption);
      });*/
